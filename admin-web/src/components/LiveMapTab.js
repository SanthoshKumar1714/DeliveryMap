...
export default function LiveMapTab() {
  ...
  return (
    <div>
      ...
      {dialogGroup && (
        <PartnerDialog group={dialogGroup} onClose={() => setDialogGroup(null)} />
      )}
    </div>
  );
}
          ...
          {group.map((p) => (
            <div key={p._id} style={ds.row}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={ds.meta}>{p.role.replace('_', ' ')} · {formatAge(p._mins)}</div>
              </div>
...
const ds = {
  overlay: {
    position: 'fixed', inset: 0, background: 'rgba(15, 26, 21, 0.55)',
    display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000,
  },
  card: {
    width: 340, maxWidth: '90vw', maxHeight: '70vh', overflow: 'hidden',
    background: '#fff', borderRadius: 14, boxShadow: '0 20px 50px rgba(0,0,0,0.35)',
    border: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column',
  },
  header: {
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    padding: '16px 18px', background: 'linear-gradient(135deg, #12261f, #1c3129)',
    borderBottom: '3px solid var(--teal, #0f7a6c)',
  },
  headerTitle: { color: '#fff', fontWeight: 700, fontSize: 15, fontFamily: 'sans-serif' },
  ...
  dot: { width: 10, height: 10, borderRadius: '50%', flexShrink: 0, boxShadow: '0 0 0 3px rgba(0,0,0,0.04)' },
  name: { fontWeight: 600, fontSize: 13.5, color: '#1f2d27', fontFamily: 'sans-serif' },
  meta: { fontSize: 11.5, color: '#6b7d74', textTransform: 'capitalize', marginTop: 1 },
